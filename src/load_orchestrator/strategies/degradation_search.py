...
class DegradationSearch(IStrategy):
    ...
    def decide(self, metrics: RawMetrics) -> Decision:
        ...
        baseline_error_rate = max(2.0, statistics.median(baseline_slice_error_rate))
        ...
        # условие деградации:
        if all(v > baseline_p95 * MULTIPLIER for v in recent_p95) or all(v > baseline_error_rate * ERROR_MULTIPLIER for v in recent_error_rate) :
